...
infix fun CharArrayAssert.hasSizeGreaterThanOrEqualTo(expected: Int): CharArrayAssert = hasSizeGreaterThanOrEqualTo(expected)
infix fun CharArrayAssert.hasSizeLessThan(expected: Int): CharArrayAssert = hasSizeLessThan(expected)
infix fun CharArrayAssert.hasSizeLessThanOrEqualTo(expected: Int): CharArrayAssert = hasSizeLessThanOrEqualTo(expected)
infix fun CharArrayAssert.hasSizeBetween(pair: Pair<Int, Int>): CharArrayAssert = hasSizeBetween(pair.first, pair.second)
infix fun CharArrayAssert.hasSameSizeAs(expected: Iterable<*>): CharArrayAssert = hasSameSizeAs(expected)
...
infix fun CharArrayAssert.containsSubsequence(values: Array<Char>): CharArrayAssert = containsSubsequence(values)
infix fun CharArrayAssert.doesNotContain(values: Array<Char>): CharArrayAssert = doesNotContain(values)
...
infix fun CharArrayAssert.isSortedAccordingTo(comparator: Comparator<Char>): CharArrayAssert = isSortedAccordingTo(comparator)
infix fun CharArrayAssert.containsExactly(values: Array<Char>): CharArrayAssert = containsExactly(values)
infix fun CharArrayAssert.containsExactlyInAnyOrder(values: Array<Char>): CharArrayAssert = containsExactlyInAnyOrder(values)
infix fun CharArrayAssert.containsAnyOf(values: Array<Char>): CharArrayAssert = containsAnyOf(values)

infix fun CharArrayAssert.contains(block: () -> Pair<Char, Index>): CharArrayAssert {
    val (value, index) = block()
    return contains(value, index)
}

infix fun CharArrayAssert.doesNotContain(block: () -> Pair<Char, Index>): CharArrayAssert {
    val (value, index) = block()
